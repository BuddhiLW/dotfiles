...
complete -c dir -l color -d 'color the output WHEN; more info below'
...
complete -c dir -s f -d 'list all entries in directory order'
complete -c dir -s F -l classify -d 'append indicator (one of */=>@|) to entries WHEN'
...
complete -c dir -l group-directories-first -d 'group directories before files; can be augmented with a --sort option, but an…'
...
complete -c dir -l dereference-command-line-symlink-to-dir -d 'follow each command line symbolic link that points to a directory'
complete -c dir -l hide -d 'do not list implied entries matching shell PATTERN (overridden by -a or -A)'
complete -c dir -l hyperlink -d 'hyperlink file names WHEN'
...
complete -c dir -s k -l kibibytes -d 'default to 1024-byte blocks for file system usage; used only with -s and per …'
...
complete -c dir -l zero -d 'end each output line with NUL, not newline'
