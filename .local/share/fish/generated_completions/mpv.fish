...
complete -c mpv -l watch-later-directory -d 'The directory in which to store the \\(dqwatch later\\(dq temporary files'
...
complete -c mpv -l watch-later-options -d 'The options that are saved in \\(dqwatch later\\(dq files if they have been cha…'
...
complete -c mpv -l player-operation-mode -d 'For enabling \\(dqpseudo GUI mode\\(dq, which means that the defaults for some …'
...
complete -c mpv -l sub-filter-regex-plain -d 'Whether to first convert the ASS \\(dqText\\(dq field to plain-text (default: n…'
...
complete -c mpv -l no-input-default-bindings -d 'Disable default-level (\\(dqweak\\(dq) key bindings'
...
complete -c mpv -l cache-pause-initial -d 'Enter \\(dqbuffering\\(dq mode before starting playback (default: no)'
...
complete -c mpv -l lavfi-complex -d 'Set a \\(dqcomplex\\(dq libavfilter filter, which means a single filter graph c…'
