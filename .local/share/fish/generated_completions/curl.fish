...
complete -c curl -l create-dirs -d 'When used in conjunction with the --output option, curl will create the neces…'
...
complete -c curl -l data-raw -d '(HTTP) This posts data similarly to --data but without the special interpreta…'
complete -c curl -l data-urlencode -d '(HTTP) This posts data, similar to the other --data options with the exceptio…'
...
complete -c curl -l doh-insecure -d 'Same as --insecure but used for DoH (DNS-over-HTTPS).   Example: '
...
complete -c curl -l form-string -d '(HTTP SMTP IMAP) Similar to --form except that the value string for the named…'
...
complete -c curl -l output-dir -d 'This option specifies the directory in which files should be stored, when --r…'
...
complete -c curl -l proxy-cert -d 'Same as --cert but used in HTTPS proxy context.   Example: '
...
complete -c curl -l proxy-insecure -d 'Same as --insecure but used in HTTPS proxy context.   Example: '
...
complete -c curl -l proxy-tlsv1 -d 'Same as --tlsv1 but used in HTTPS proxy context.   Example: '
...
complete -c curl -s J -l remote-header-name -d '(HTTP) This option tells the --remote-name option to use the server-specified…'
...
complete -c curl -l suppress-connect-headers -d 'When --proxytunnel is used and a CONNECT request is made do not output proxy …'
