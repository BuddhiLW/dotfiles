...
complete -c openvpn -l providers -d 'Load the list of (OpenSSL) providers'
...
complete -c openvpn -l ncp-disable -d 'DEPRECATED Disable \\(dqNegotiable Crypto Parameters\\(dq'
...
complete -c openvpn -l push-remove -d 'Selectively remove all --push options matching \\(dqopt\\(dq from the option li…'
...
complete -c openvpn -l vlan-pvid -d 'Specifies which VLAN identifier a \\(dqport\\(dq is associated with'
