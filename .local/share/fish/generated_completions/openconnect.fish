...
complete -c openconnect -s c -l certificate -l mca-certificate -d 'Use SSL client certificate  CERT which may be either a file name or, if OpenC…'
...
complete -c openconnect -s k -l sslkey -l mca-key -d 'Use SSL private key  KEY which may be either a file name or, if OpenConnect h…'
...
complete -c openconnect -l force-dpd -d 'Use  INTERVAL as Dead Peer Detection interval (in seconds)'
...
complete -c openconnect -s l -l syslog -d 'After tunnel is brought up, use syslog for further progress messages'
...
complete -c openconnect -s p -l key-password -l mca-key-password -d 'Provide passphrase for certificate file, or SRK (System Root Key) PIN for TPM…'
...
complete -c openconnect -s Q -l queue-len -d 'Set packet queue limit to  LEN packets.  The default is 10'
...
complete -c openconnect -l server -d 'Define the VPN server as a simple  HOST or as an URL containing the '
...
complete -c openconnect -l authenticate -d 'Authenticate to the VPN, output the information needed to make the connection…'
complete -c openconnect -l cookieonly -d 'Fetch and print cookie only; don\'t connect (this is essentially a subset of  …'
complete -c openconnect -l printcookie -d 'Print cookie to stdout before connecting (see  --authenticate for the meaning…'
...
complete -c openconnect -l allow-insecure-crypto -d 'The ancient, broken 3DES and RC4 ciphers are insecure; we explicitly disable …'
...
complete -c openconnect -l reconnect-timeout -d 'After disconnection or Dead Peer Detection, keep trying to reconnect for R SE…'
complete -c openconnect -l resolve -d 'Automatically resolve the hostname R HOST to R IP instead of using the normal…'
complete -c openconnect -l servercert -d 'Accept server\'s SSL certificate only if it matches the provided fingerprint'
