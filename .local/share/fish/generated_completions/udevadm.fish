...
complete -c udevadm -l property -d 'When showing device properties using the --query=property option, limit displ…'
complete -c udevadm -l value -d 'When showing device properties using the --query=property option, print only …'
...
complete -c udevadm -s t -l tree -d 'Display a sysfs tree'
...
complete -c udevadm -l type -d 'Trigger a specific type of devices'
complete -c udevadm -l action -d 'Type of event to be triggered'
complete -c udevadm -l prioritized-subsystem -d 'Takes a comma separated list of subsystems'
...
complete -c udevadm -l initialized-match -l initialized-nomatch -d 'When --initialized-match is specified, trigger events for devices that are al…'
...
complete -c udevadm -l initialized -d 'Check if systemd-udevd initialized devices.  Defaults to true'
complete -c udevadm -l removed -d 'When specified, the command wait for devices being removed instead of created…'
complete -c udevadm -l device -d 'Takes a path to a device node of the device to lock'
complete -c udevadm -l backing -d 'If a path to a device node is specified, identical to --device='
complete -c udevadm -l print -d 'Instead of locking the specified device(s) and executing a command, just prin…'
