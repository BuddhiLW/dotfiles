...
complete -c scp -s D -d 'When using the SFTP protocol support via M, connect directly to a local SFTP …'
...
complete -c scp -s O -d 'Use the legacy SCP protocol for file transfers instead of the SFTP protocol'
