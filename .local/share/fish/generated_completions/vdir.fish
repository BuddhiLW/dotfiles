...
complete -c vdir -l color -d 'color the output WHEN; more info below'
...
complete -c vdir -s f -d 'list all entries in directory order'
complete -c vdir -s F -l classify -d 'append indicator (one of */=>@|) to entries WHEN'
...
complete -c vdir -l group-directories-first -d 'group directories before files; can be augmented with a --sort option, but an…'
...
complete -c vdir -l dereference-command-line-symlink-to-dir -d 'follow each command line symbolic link that points to a directory'
complete -c vdir -l hide -d 'do not list implied entries matching shell PATTERN (overridden by -a or -A)'
complete -c vdir -l hyperlink -d 'hyperlink file names WHEN'
...
complete -c vdir -s k -l kibibytes -d 'default to 1024-byte blocks for file system usage; used only with -s and per …'
...
complete -c vdir -l zero -d 'end each output line with NUL, not newline'
complete -c vdir -s 1 -d 'list one file per line'
