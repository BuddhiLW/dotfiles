...
** Claude Code
Integration with the [[https://github.com/anthropics/claude-code][Claude Code CLI]] directly from Emacs using [[https://github.com/stevemolitor/claude-code.el][claude-code.el]].

Features:
- Multi-backend terminal support (eat/vterm)
- Multiple named instances per project
- Send regions, buffers, and errors to Claude
- Transient menus for organized command access
- Hook integration for bidirectional CLI communication
...
*** Usage
- =SPC b ai C= → Start Claude Code session
- =SPC b ai x= → Continue previous conversation
- =SPC b ai R= → Resume specific past session
- =SPC b ai M= → Claude Code transient menu
- =SPC b ai S= → Send region/buffer to Claude
- =SPC b ai F= → Fix flycheck/flymake error at point
- =C-c C= → Quick access to Claude Code transient menu

In a Claude buffer (Interactive Mode):
- =RET= → Send message
- =S-RET= → Insert newline
- =C-g= → Send ESC to Claude (cancel/no)
- =jk= → Toggle read-only mode (Evil-style escape)
- =C-z= → Toggle read-only mode (alternative)
- =C-c C-g= → Real keyboard-quit
