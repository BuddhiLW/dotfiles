...
Polyfill =set-local= on pre-release Emacs (e.g. 31.0.50): compat only
defines it when =(version< emacs-version "31")=, which is false for a
=31.0.x= dev build even though core hasn't landed =set-local= yet.
vertico 2.10 calls it and would signal =void-function set-local=.
#+begin_src emacs-lisp
...
#+RESULTS:
| ~/PP/self-managment/Agenda/* | ~/PP/self-managment/README.org |
...
#+RESULTS:
| project-find-go-module | project-projectile | project-try-vc |
...
*** hive-mcp AI Bridge
Integration between hive-mcp memory/swarm and Emacs AI ecosystem.
...
*** Integration with Claude MCP for project memory, context, and workflows.
...
** hive-emacs-hagent
Headed-agent chat backends (openclaude, claude agent CLI) written in cljel.
Sessions register into =hive-mcp-swarm--slaves= so they appear in the Olympus
Grid dashboard alongside real lings.
...
*** Fix: force initial fontify when indent-bars enables in -ts-mode buffers
indent-bars-ts wraps font-lock-fontify-buffer-function and inhibits font-lock
unless its ~indent-bars-font-lock-pending~ text property is set. On first load
the property is never set on the existing buffer text, so jit-lock skips
fontification \rightarrow buffer renders with no faces. Mark the whole buffer pending
and trigger jit-lock fontification synchronously after the mode is fully set
up (deferred so indent-bars-ts has wired in its hooks).
...
#+RESULTS:
| indent-bars-mode | rainbow-delimiters-mode | elm-indent-mode |
...
This section configures multiple AI tools and models:

** Local Ollama Models:
- *deepseek-r1:latest* → Hard reasoning, multi-file refactors, complex analysis
- *qwen3:latest* → General coding assistant, Go/Clojure/TypeScript 
- *llama3.2:latest* → Quick drafts, low latency interactions
- *gemma3:latest* → Summarization, documentation polish
- *gpt-oss:latest* → Fallback generalist, agentic tasks
- *nomic-embed-text:latest* → Embeddings

** AI Pair Programming:
- *Aidermacs* → AI pair programming with built-in Ediff integration, uses OpenRouter's free `nousresearch/deephermes-3-llama-3-8b-preview:free` model
- *GPTel* → Direct chat interface with multiple model support
- *Ellama* → Specialized AI tasks (coding, summarization, translation)
- *Claude Code* → Full Claude Code CLI integration with terminal embedding, session management

** Usage

- =SPC b ai m= → Open LLM transient menu
- =SPC b ai c= → Quick gptel chat
- =SPC b ai r= → Auto-route model based on content
- =SPC b ai s= → Manually select model
- =SPC b ai e= → Ellama transient menu
- =SPC b ai E= → Ellama chat
- =SPC b ai a= → Aidermacs (AI Pair Programming)
- =SPC b ai A= → Aidermacs transient menu
- =C-c A= → Quick access to Aidermacs menu
- =SPC b ai C= → Claude Code session
- =SPC b ai M= → Claude Code transient menu
- =C-c C= → Quick access to Claude Code menu

** Auto-routing Logic

The system automatically selects models based on:
- Content length (>1500 chars → deepseek-r1)
- Keywords (refactor/test/debug → deepseek-r1)
- Major mode (go/clojure/typescript → qwen3)
- Task type (summarize/polish → gemma3)
- Default (llama3.2 for speed)
...
** ELLAMA
Use =ollama= inside Emacs with specialized providers for different tasks!
...
** org-ai
AI blocks in org-mode for inline LLM interactions.
...
** Aidermacs
AI Pair Programming in Emacs with Aider - a more Emacs-native experience with enhanced features:

- **Built-in Ediff Integration**: Automatically shows diffs for AI-modified files
- **Intelligent Model Selection**: Automatic discovery and compatibility checking
- **Flexible Terminal Backends**: Support for both comint and vterm
- **Enhanced File Management**: Add/drop files, view history, create scratchpads
- **Multiline Input Support**: Natural handling of complex prompts
- **Community-Driven**: Actively developed with user feedback
...
** Claude Code
Integration with the [[https://github.com/anthropics/claude-code][Claude Code CLI]] from Emacs via [[https://github.com/stevemolitor/claude-code.el][claude-code.el]] (vterm-only since the eat backend was dropped upstream).

Features (current upstream surface):
- vterm-based session per project
- Send regions and prepared prompts to Claude
- MCP tools / events bridge to Emacs
...
*** Usage
- =SPC b ai C= → Start Claude Code session for current project (prefix arg = pick CLI option)
- =SPC b ai x= → Switch to existing Claude buffer
- =SPC b ai K= → Close Claude session
- =SPC b ai Q= → Quit Claude (force)
- =SPC b ai S= → Send region to Claude
- =SPC b ai T= → Compact current conversation
- =C-c C= → Open project prompt file

In a Claude buffer (vterm):
- =RET= → Send message (vterm-send-return)
- =S-RET= → Insert newline (claude-code-newline-keybinding-style if customized)
- =C-c C-c= → claude-code-send-escape (cancel/no)
- Use vterm's =copy-mode= (=C-c C-t=) for read-only navigation

In a Claude buffer (Read-Only Mode):
- Normal Evil navigation (=j=, =k=, =gg=, =G=, =/=, etc.)
- =y= to yank text
- =ESC= → Return to interactive mode
- =C-z= → Return to interactive mode
** ECA
