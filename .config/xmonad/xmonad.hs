...
--     output <- io $ readProcess "xrandr" ["--query"] ""
--     let connectedMonitors = filter (isInfixOf " connected") $ lines output
--         numMonitors = length connectedMonitors
--         laptop = "eDP-1"
...
--         2 -> do
--             -- Find external monitor (not laptop)
--             let extMonitor = head [head (words line) | line <- connectedMonitors, not $ laptop `isInfixOf` line]
--                 -- Get preferred mode for external monitor
--                 extMode = head [head (words line) | line <- lines output,
--                               take (length extMonitor) line == extMonitor,
--                               "*" `isInfixOf` line]

--             spawn $ unwords
--                 [ "xrandr --output", laptop, "--primary --mode 1920x1200 --pos 0x0 --rotate normal"
--                 , "--output", extMonitor, "--mode", extMode, "--pos 1920x0 --rotate normal"
--                 , "--output DP-1-0 --off"
--                 , "--output DP-1-1 --off"
--                 , "--output DP-1-2 --off"
--                 , "--output DP-1 --off"
...
myStartupHook = do
    ...
    -- -- Run the monitor setup with a small delay
    -- spawn "sleep 2" >> setupMonitors
