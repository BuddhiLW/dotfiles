...
-- Setup monitor in `$HOME/.profile` -> add a script like `/usr/bin/sh "$HOME/.screenlayout/default.sh"`
-- Where `default.sh` is the saved - autogenerated - output of your arandr setup

-- Update your startupHook to use the new function
myStartupHook :: X ()
myStartupHook = do
...
showKeybindings x = addName "Show Keybindings" $ io $ do
  h <- spawnPipe $ "yad --text-info --fontname=\"FontAwesome 40\" --fore=#46d9ff back=#282c36 --center --geometry=1200x800 --title \"XMonad keybindings\""
...
--   , ("M-p t", addName "Translate text"         $ spawn "dm-translate")]
  ...
  -- , ("M-S-d", addName "Detect and setup monitors" $ setupMonitors)
  ]
  ...
  , ("M-e n", addName "Emacsclient Dashboard"    $ spawn "emacsclient -a emacs -c")
  ...
  -- , ("M-e a", addName "Emacsclient EMMS (music)" $ spawn "emacsclient -a emacs -c")
...
main = do
  ...
  xmonad
    ...
    , logHook = dynamicLogWithPP $  filterOutWsPP [scratchpadWorkspaceTag] $ xmobarPP
        ...
        , ppTitle = xmobarColor color16 "" . shorten 38
          -- Separator character
